...
```{r}
# STOCK PRICES
sp_500_prices_tbl <- read_rds("./01_ml_fund_source/Business\ Decisions\ with\ Machine\ Learning/sp_500_prices_tbl.rds")
sp_500_prices_tbl
```
...
```{r}
# SECTOR INFORMATION
sp_500_index_tbl <- read_rds("./01_ml_fund_source/Business\ Decisions\ with\ Machine\ Learning/sp_500_index_tbl.rds")
sp_500_index_tbl
```
...
sp_500_daily_returns_tbl<-sp_500_prices_tbl %>%
    ...
    group_by(symbol) %>%
    mutate(lag_adjusted = lag(adjusted, n = 1, default = NA)) %>% 
    drop_na() %>%
...
```{r}
sp_500_daily_returns_tbl <- read_rds("./01_ml_fund_source/Business\ Decisions\ with\ Machine\ Learning/sp_500_daily_returns_tbl.rds")
...
stock_date_matrix_tbl %>% arrange(symbol) %>% head(5)
...
We're going to import the correct results first (just in case you were not able to complete the last step).
```{r}
stock_date_matrix_tbl <- read_rds("./01_ml_fund_source/Business\ Decisions\ with\ Machine\ Learning/stock_date_matrix_tbl.rds")
...
#stock_date_matrix_tbl %>% select(-c("symbol")) %>% head() filter not work
kmeans_obj <- subset(stock_date_matrix_tbl, select = -c(symbol)) %>%
...
kmeans_mapper <- function(center = 3) {
        subset(stock_date_matrix_tbl, select = -c(symbol)) %>%
...
We're going to import the correct results first (just in case you were not able to complete the last step).
```{r}
k_means_mapped_tbl <- read_rds("./01_ml_fund_source/Business\ Decisions\ with\ Machine\ Learning/k_means_mapped_tbl.rds")
k_means_mapped_tbl %>% head(5)
```
...
```{r}
stock_date_matrix_tbl %>% select(symbol) %>% head(5)
```
...
```{r}
k_means_mapped_tbl <- read_rds("./01_ml_fund_source/Business\ Decisions\ with\ Machine\ Learning/k_means_mapped_tbl.rds")
umap_results_tbl   <- read_rds("./01_ml_fund_source/Business\ Decisions\ with\ Machine\ Learning/umap_results_tbl.rds")
umap_results_tbl %>% head(5)
k_means_mapped_tbl %>% head(5)
```
